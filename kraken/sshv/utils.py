...
class SSHConn(object):
    ...
    def down_interface(self, device):
        ...
        if result["st"]:
            prt_log(None, "Start to shutdown interface: %s" % device, 0)
            return True
    ...
    def up_interface(self, device):
        ...
        result = self.exec_cmd(cmd)
        if result["st"]:
            prt_log(None, "start to opened interface: %s" % device, 0)
            return True

    def check_interface(self, device):
        cmd = f"ifconfig {device}"
        result = self.exec_cmd(cmd)
        if result["st"]:
            if "RUNNING" in result["rt"]:
                return 0
            else:
                return 1
        else:
            return 2
            prt_log(None, "Get interface %s information failed!!!" % device,0)

    def down_ipmi(self):

        cmd = f"ipmitool -I lanplus -H 10.203.1.230 -U admin -P admin power off"
        prt_log(None, "Ipmi start shutdown node... ",0)
        result = self.exec_cmd(cmd)
        print("info:rt")
        print(result["rt"])
        if result["st"]:
            if "Power Control: Down" in result["rt"]:
                prt_log(None, "Node has been down... ",0)
            else:
                prt_log(None, "ipmi down node failed!!!",0)
        else:
            prt_log(None, "ipmi down node failed!!!",0)
    ...
    def on_ipmi(self):
        ...
        cmd = f"ipmitool -I lanplus -H 10.203.1.230 -U admin -P admin power on"
        result = self.exec_cmd(cmd)
        if result["st"]:
            if "Power Control: Up" in result["rt"]:
                prt_log(None, "Node has been power on... ",0)
            else:
                prt_log(None, "ipmi power on node failed!!!",0)
        else:
            prt_log(None, "ipmi power on node failed!!!",0)
...
class ConfFile(object):
    ...
    def get_down_node(self):
        ...
        bmc_ip = down_host["bmc_ip"]
        user = down_host["user"]
        bmc_password = down_host["bmc_password"]
        return {"hostname":hostname,"port":port,"ip":ip,"password":password,"bmc_ip":bmc_ip,"user":user,"bmc_password":bmc_password}
    @deco_yaml_dict
